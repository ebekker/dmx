...
/// <summary>
/// Based on:
///   https://github.com/Blazor-Diagrams/Blazor.Diagrams/blob/develop/src/Blazor.Diagrams.Core/Routers/OrthogonalRouter.cs
/// but adapted for Nodes/Anchors.
/// </summary>
public class OrthoAnchorRouter : Router
{
    private readonly Router _fallbackRouter;
    private readonly double _shapeMargin;
    private readonly double _globalMargin;

    public OrthoAnchorRouter(double shapeMargin = 10d, double globalMargin = 50d, Router? fallbackRouter = null)
    {
    ...
    public override Point[] GetRoute(Diagram diagram, BaseLinkModel link)
    {
        ...
        //if (link.Source is not SinglePortAnchor childAnchor)
        //    return _fallbackRouter.GetRoute(diagram, link);
        if (link.Source is not EntityAnchor childAnchor)
            return _fallbackRouter.GetRoute(diagram, link);

        //if (link.Target is not SinglePortAnchor parentAnchor)
        //    return _fallbackRouter.GetRoute(diagram, link);
        if (link.Target is not EntityAnchor parentAnchor)
            return _fallbackRouter.GetRoute(diagram, link);

        //var sourcePort = childAnchor.Port;
        //if (parentAnchor == null || sourcePort.Parent.Size == null || parentAnchor.Port.Parent.Size == null)
        //    return _fallbackRouter.GetRoute(diagram, link);
        if (parentAnchor == null || childAnchor.Node.Size == null || parentAnchor.Node.Size == null)
            return _fallbackRouter.GetRoute(diagram, link);

        //var targetPort = parentAnchor.Port;
        ...
        var sideA = childAnchor.GetAlignment(link);
        var sideAVertical = IsVerticalSide(sideA);
        //var sideB = targetPort.Alignment;
        var sideB = parentAnchor.GetAlignment(link);
        ...
        var originA = childAnchor.GetPosition(link, []);
        var originB = parentAnchor.GetPosition(link, []);

        //var shapeA = sourcePort.Parent.GetBounds(includePorts: true)!;
        //var shapeB = targetPort.Parent.GetBounds(includePorts: true)!;
        var shapeA = childAnchor.Node.GetBounds(includePorts: true)!;
        var shapeB = parentAnchor.Node.GetBounds(includePorts: true)!;
